...
use crate::hint_processor::hint_processor::Cairo1HintProcessor;
...
pub fn cairo_run_program(
...
) -> (CairoRunner, Vec<MaybeRelocatable>, Option<String>) {
    let metadata = calc_metadata_ap_change_only(sierra_program).unwrap();
    ...
    let (entry_code, builtins) = match create_entry_code(
        &sierra_program_registry,
        &casm_program,
        &type_sizes,
        main_func,
        &cairo_run_config,
    ) {Ok((entry_code, builtins)) => (entry_code, builtins),
        Err(e) => {
            panic!("Error creating entry code: {:?}", e);
        }
    };
    ...
    load_arguments(&mut runner, &cairo_run_config, main_func).unwrap();
    ...
    return (runner, return_values, serialized_output);
}
        ...
        match arg {
            FuncArg::Array(args) => {
                ...
                runner.vm.insert_value(
                    (runner.vm.get_ap() + ap_offset).unwrap(),
                    array_start,
                ).unwrap();
                ap_offset += 1;
                runner.vm.insert_value(
...
fn create_entry_code(
    ...
    for ty in &signature.param_types {
        ...
        if let Some(var) = builtin_vars.get(generic_ty).cloned() {
            ...
            // Load initial gas
            casm_build_extend! {ctx,
                const initial_gas = 9_999_999_999_999_u64;
...
fn fetch_return_values(
    ...
    if fetch_from_output {
        ...
        let return_values =
            vm.get_continuous_range((2, 0).into(), vm.get_segment_size(2).unwrap()).unwrap();
    ...
    if let Some(inner_type_size) = result_inner_type_size {
        ...
        if return_values.first() != Some(&MaybeRelocatable::from(0)) {
            ...
            let panic_data = vm.get_integer_range(
                panic_data_start,
                (panic_data_end - panic_data_start).map_err(VirtualMachineError::Math).unwrap(),
            ).unwrap();
            return Err(Error::RunPanic(
...
fn finalize_builtins(
    ...
    // Set stop pointer for each builtin
    vm.builtins_final_stack_from_stack_pointer_dict(&builtin_name_to_stack_pointer, false).unwrap();
    Ok(())
}
